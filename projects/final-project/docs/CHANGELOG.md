...
### Cambiado
- **Política AWS IoT de PI-4** (`PI-4/Policy.json`) migrada a `${iot:Connection.Thing.ThingName}` (alineada con la de PI-5); corregido el typo `Pi4-felix`→`Pi4-Felix` en `PI-5/Policy.json`.
- **Monolito `agente_monitor3.py` retirado**: eliminados los 7 artefactos solo-legado (el sensor, su Dockerfile/systemd/setup, su `config.yml` desincronizado, la copia muerta de `aws_connector.py` y su `requirements.txt`). La política IoT del device queda en `PI-4/Policy.json` y la clave pública de firma en `PI-5/sentinel_pi5_signing.pub`; el verificador Ed25519 vive solo en el sensor (`sentinel-agent`). Docs actualizados (`funcionamiento_mqtt.md`, `Testing_Guide.md`, `Configuration_and_Deployment.md`, `diseno_agente_discovery.md` y la ficha histórica de PI-4) para reflejar que el sensor vigente es `sentinel-agent`.
- **Nomenclatura más clara:** la carpeta del antiguo agente `PI-4/Agente de monitorizacion/` se eliminó (su `Policy v2.json` → `PI-4/Policy.json`, la clave pública de firma → `PI-5/`, y el verificador duplicado se descartó en favor del del sensor). La web honeypot pasó de `PI-4/Pagina web/` a `PI-4/pagina-web/` (sin espacios). Referencias en código (`generate_signing_keys.py`, `test_signing.py`) y docs actualizadas.

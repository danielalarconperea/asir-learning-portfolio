...
### Cambiado
- **Política AWS IoT de PI-4** (`PI-4/firma-iot/Policy.json`) migrada a `${iot:Connection.Thing.ThingName}` (alineada con la de PI-5); corregido el typo `Pi4-felix`→`Pi4-Felix` en `PI-5/Policy.json`.
- **Monolito `agente_monitor3.py` retirado**: eliminados los 7 artefactos solo-legado (el sensor, su Dockerfile/systemd/setup, su `config.yml` desincronizado, la copia muerta de `aws_connector.py` y su `requirements.txt`). Se conservan `signing.py`, `sentinel_pi5_signing.pub` y `Policy.json` (atados a código vivo) con un README de deprecación. Docs actualizados (`funcionamiento_mqtt.md`, `Testing_Guide.md`, `Configuration_and_Deployment.md`, `diseno_agente_discovery.md` y la ficha histórica de PI-4) para reflejar que el sensor vigente es `sentinel-agent`.
- **Nomenclatura más clara:** la carpeta `PI-4/Agente de monitorizacion/` (ya sin agente) se renombró a `PI-4/firma-iot/` y `Policy v2.json` → `Policy.json`; la web honeypot pasó de `PI-4/Pagina web/` a `PI-4/pagina-web/` (sin espacios). Referencias en código (`generate_signing_keys.py`, `test_signing.py`) y docs actualizadas.

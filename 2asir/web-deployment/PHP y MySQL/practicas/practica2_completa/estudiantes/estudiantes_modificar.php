...
if ($conexion) {
    ...
    // Si viene un ID por GET, forzar la búsqueda de ese estudiante específico
    if (isset($_GET['id']) && !isset($_POST['buscar_est'])) {
        $id_est_get = mysqli_real_escape_string($conexion, $_GET['id']);
        $consulta = "SELECT * FROM Estudiantes WHERE id_estudiante = '$id_est_get'";
    ...
    if (isset($_POST['buscar_est']) && !empty(trim($_POST['query']))) {
        $busqueda = mysqli_real_escape_string($conexion, trim(strtolower($_POST['query'])));
        $consulta = "SELECT * FROM Estudiantes WHERE LOWER(nombre) LIKE '%$busqueda%' OR LOWER(apellidos) LIKE '%$busqueda%' OR
        LOWER(codigo_estudiante) LIKE '%$busqueda%'";
    } else {
            ...
            <section class="content">
                ...
                <form action="estudiantes_modificar.php" method="post">
                    <div class="form-group">
                        ...
                        <input type="text" id="query" name="query" placeholder="Nombre, apellido o código...">
                    </div>

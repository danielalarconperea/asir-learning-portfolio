...
if ($conexion) {
    ...
    // Si viene un ID por GET, forzar la búsqueda de ese préstamo específico
    if (isset($_GET['id']) && !isset($_POST['buscar_pr'])) {
        $id_pr_get = mysqli_real_escape_string($conexion, $_GET['id']);
        $consulta = "SELECT P.id_prestamo, L.titulo AS titulo_libro, E.nombre, E.apellidos, P.fecha_prestamo,
                    P.fecha_devolucion, P.devuelto
                    FROM Prestamos P
                    JOIN Libros L ON P.id_libro = L.id_libro
                    JOIN Estudiantes E ON P.id_estudiante = E.id_estudiante
                    WHERE P.id_prestamo = $id_pr_get";
    ...
    if (isset($_POST['modificar'])) {
        ...
        // Si el préstamo está devuelto, el libro está disponible (1). Si no, no está disponible (0).
                $nuevo_estado_disp = ($devuelto == 1) ? 1 : 0;
                mysqli_query($conexion, "UPDATE Libros SET disponible = $nuevo_estado_disp WHERE id_libro = $id_lib");
            }
        } else {
    ...
    if (isset($_POST['buscar_pr']) && !empty(trim($_POST['query']))) {

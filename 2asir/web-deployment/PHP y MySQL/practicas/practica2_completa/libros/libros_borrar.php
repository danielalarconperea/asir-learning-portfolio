...
mysqli_set_charset($conexion, "utf8mb4");
    if (isset($_POST['id_a_borrar']) || isset($_GET['id'])) {
        $id_a_borrar = isset($_POST['id_a_borrar']) ? intval($_POST['id_a_borrar']) : intval($_GET['id']);
        $consulta = "DELETE FROM Libros WHERE id_libro = $id_a_borrar";
        if (mysqli_query($conexion, $consulta)) {
            echo "<div class='success-message'>Libro eliminado correctamente.</div>";
        } else {
            echo "<div class='error-message'>Error al eliminar: " . mysqli_error($conexion) . "</div>";
        }
    }
}
...
if (!$conexion) {
    ...
    if (isset($_POST['buscar'])) {
        ...
        $consulta = "SELECT * FROM Libros WHERE LOWER(titulo) LIKE '%$busqueda%' OR LOWER(autor) LIKE '%$busqueda%' OR
        LOWER(editorial) LIKE '%$busqueda%' OR LOWER(isbn) LIKE '%$busqueda%' OR LOWER(anio_publicacion) LIKE '%$busqueda%'";
    } else {
    ...
    <div class="container">
        ...
        <main>
            ...
            <section class="content">
                ...
                <form action="libros_borrar.php" method="post" style="margin-bottom: 30px;">
                ...
                <div class="table-container">
                    <?php if (isset($error_detalle)) { ?>
                        <div class="error-message"
                            ...
                            <strong style="display: block; margin-bottom: 10px;">Error de Sistema</strong>

/*
PROBLEMA 1: Reporte de ventas por empleado y categoría

El restaurante necesita analizar el rendimiento de sus camareros y los productos más rentables.
Crea un procedimiento que genere un reporte detallado de ventas por empleado (simulado por cliente en este caso) 
y categoría de producto. El reporte debe incluir métricas como unidades vendidas, ingresos totales, 
comisión calculada (5% sobre ventas mayores a 10€) y porcentaje de contribución.

Crea un procedimiento llamado sp_reporte_ventas_empleado_categoria que:
1. Reciba como parámetro un rango de fechas (fecha_inicio y fecha_fin, tipo DATE).
2. Genere un informe que muestre:
   - Nombre del cliente (empleado)
   - Categoría del producto
   - Total de unidades vendidas
   - Ingreso total generado
   - Comisión calculada (5% solo sobre ventas donde el subtotal por producto > 10€)
   - Porcentaje del ingreso total del periodo

Ejemplo de llamada:
CALL sp_reporte_ventas_empleado_categoria('2024-04-01', '2024-04-07');
*/
...
BEGIN
    ...
    SELECT
        ...
        SUM(CASE WHEN dp.subtotal > 10 THEN dp.subtotal * 0.05 ELSE 0 END) AS comision,
        (SUM(dp.subtotal) / (SELECT SUM(subtotal) FROM detalle_pedido WHERE fecha BETWEEN fecha_inicio AND fecha_fin)) * 100 AS porcentaje_ingreso
    FROM detalle_pedido dp
    JOIN clientes c ON dp.id_cliente = c.id_cliente
    JOIN productos p ON dp.id_producto = p.id_producto
    JOIN categorias cat ON p.id_categoria = cat.id_categoria
    WHERE dp.fecha BETWEEN fecha_inicio AND fecha_fin
    GROUP BY c.nombre, cat.nombre;
